...
public class EmojiBuilder
{
    ...
    {
        EmojiAsset asset = Selection.activeObject as EmojiAsset;
        if(asset == null) return;
        
        Material material = new Material(Shader.Find("UI/EmojiFont"));
        //AssetDatabase.AddObjectToAsset(material, asset);

        material = AssetDatabase.LoadAssetAtPath<Material>("Assets/EmojiTextTest/TestProject/Hidden_UI_Emoji.mat");
        ...
        // string mPath = path.Replace(ext, ".mat");
        // AssetDatabase.CreateAsset(material, mPath);
    ...
    static void CreateEmojiAsset(string path, Texture2D[] textures)
    {
        ...
        string materialPath = path.Replace(ext, ".mat");
        AddDefaultMaterial(emojiAsset, materialPath);
    ...
    static void AddDefaultMaterial(EmojiAsset asset, string path)
    {
        if(null == asset)
        {
            return;
