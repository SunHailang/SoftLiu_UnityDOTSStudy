...
namespace act.ui.EmojiText
{
    ...
    public class EmojiManager : MonoBehaviour
    {
        ...
        //绘制的模型数据信息
        private readonly Dictionary<int, Dictionary<UiEmojiText, MeshInfo>> _graphicMeshInfo = new Dictionary<int, Dictionary<UiEmojiText, MeshInfo>>();
        ...
        private void Update()
        {
            if (_renderIndexs != null && _renderIndexs.Count > 0)
            {
                for (int i = 0; i < _renderIndexs.Count; i++)
                {
                    ...
                    {
                        if (!_graphicMeshInfo.TryGetValue(id, out Dictionary<UiEmojiText, MeshInfo> textMeshInfo))
                        {
        ...
        //更新Text文本信息
        public void UpdateTextInfo(UiEmojiText key, int id, List<SpriteTagInfo> value, bool visable)
        {
            Dictionary<UiEmojiText, MeshInfo> textMeshInfo;
            if (value == null)
            {
            ...
            else
            {
                ...
                if (spriteGraphic != null)
                {
                    ...
                    {
                        textMeshInfo = new Dictionary<UiEmojiText, MeshInfo>();
